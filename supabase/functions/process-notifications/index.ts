...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const notification of pending) {
      ...
      const title = notification.type === "lembrete" ? "⏰ Aula em 20 minutos!" : "✅ A aula foi realizada?";
      const body = notification.type === "lembrete"
        ? `Aula com ${clientName} às ${startTime}. Prepare-se!`
        : `Aula com ${clientName} — foi concluída?`;
      const payload = JSON.stringify({
        title, body,
        data: { type: notification.type === "lembrete" ? "lembrete" : "completion", sessionId: notification.session_id }
      });
      for (const sub of subscriptions) {
        try {
          await webpush.sendNotification(sub.subscription, payload);
          console.log(`[process-notifications] Push sent: ${title} to ${notification.user_id}`);
        } catch (err: any) {
          console.error(`[process-notifications] Push error: ${err.message}`);
        }
      }
      await supabase.from("scheduled_notifications").update({ sent: true }).eq("id", notification.id);
      processedCount++;
    }
    console.log(`[process-notifications] Done. Processed: ${processedCount}`);
